---
title: "CBDA-SL and Knockoff Filter Results on the Binomial Dataset - n=300 p=1500, 9000 jobs x 6 experiment - MSE and Accuracy as ranking metrics for the CBDA"
...
# Binomial_dataset_5_new = read.csv("C:/Users/simeonem/Documents/CBDA-SL/ExperimentsNov2016/Binomial/Binomial_5/Binomial_dataset_5.txt",header = TRUE)
...
label=c("Binomial_dataset_5_final")
...
eval(parse(text=paste0("save(arguments,workspace_directory,list_exps,label,file= \"~/temp_Binomial_dataset_5_new.RData\")")))
#print(getwd())
```
...
#     }
  ...
  ## DEFINE HERE THE TOP # OF COVARIATES TO LIST in the MODEL MINING STEP
  # "top" is defined at the beginning (line 8) and represents the top Accuracys to consider for
  # feature mining (ks). Each one will have a set of best features with their relative highest frequencies
  #top=100
...
eval(parse(text=paste0("h_acc=hist(k_top_",top,"_temp_Accuracy, plot = FALSE ,breaks=seq(min(k_top_",top,"_temp_Accuracy)-0.5, max(k_top_",top,"_temp_Accuracy)+0.5, by=1))")))
h_acc$density = h_acc$counts/sum(h_acc$counts)*100
title_temp <- c("CBDA-SL RESULTS - Accuracy metric")
#print(title_temp)
plot(h_acc,freq=FALSE,ylab='Density (%)',xlab='Feature #',main = title_temp,ylim=c(0,max(h$density)))

#histbackback(list(KO=h$density,CBDA-accuracy=h_acc$density), probability=TRUE)#,ylim=c(-1500,1500))
#,xlab(c("KO","CBDA-Accuracy")),ylab("Features"))

# RETRIEVE AND SAVE THE LABELS OF THE TOP [BEST] FEATURES
 Binomial_dataset_5_new = read.csv("Binomial_dataset_5_final.txt",header = TRUE)
...
cols_to_eliminate=c(1)
  ...
  Top_features_labels <- names(Binomial_dataset_5_new[-cols_to_eliminate])[as.numeric(names(Top_features))]
  Top_features_labels_MSE <- names(Binomial_dataset_5_new[-cols_to_eliminate])[as.numeric(names(Top_features_MSE))]
  
  eval(parse(text=paste0("Top_",BEST,"_features_labels <- names(Binomial_dataset_5_new[-cols_to_eliminate])[as.numeric(names(Top_features)[1:",BEST,"])]")))
  eval(parse(text=paste0("Top_",BEST,"_features_labels_MSE <- names(Binomial_dataset_5_new[-cols_to_eliminate])[as.numeric(names(Top_features_MSE)[1:",BEST,"])]")))
...
qa_MSE <-as.data.frame(Top_features_MSE[1:top])
...
if (is.null(KO_sub) == FALSE)
{
  ...
  qa_ALL$KO_Count <- Top_Knockoff_features[1:top]
  qa_ALL$KO_Density <- 100*(Top_Knockoff_features[1:top]/sum(Top_Knockoff_features))
...
print(qa_ALL[1:top,], right = FALSE, row.names = FALSE)

#Binomial_dataset_5_final_MSE
...
eval(parse(text=paste0("Binomial_dataset_5_new_exp_",i,"<-qa")))
...
eval(parse(text=paste0("save(Binomial_dataset_5_new_exp_",i,",arguments,workspace_directory,list_exps,label,file= \"Binomial_dataset_5_new_exp_",i,".RData\")")))
# print(q)
rm(list = ls())
eval(parse(text=paste0("load(\"~/temp_Binomial_dataset_5_new.RData\")")))
cat("\n\n\n\n\n\n")
}
```

```{r GENERATE THE COMBINED TABLE OF RESULTS ACROSS EXPERIMENTS, echo=FALSE, eval = FALSE}
eval(parse(text=paste0("load(\"~/temp_Binomial_dataset_5_new.RData\")")))
Binomial_dataset_5_new_ALL <- NULL
for (i in list_exps) {
  eval(parse(text=paste0("load(\"Binomial_dataset_5_new_exp_",i,".RData\")")))
  eval(parse(text=paste0("Binomial_dataset_5_new_ALL <- rbind(Binomial_dataset_5_new_ALL,Binomial_dataset_5_new_exp_",i,")")))
}
eval(parse(text=paste0("save(Binomial_dataset_5_new_ALL,arguments,workspace_directory,list_exps,label,file= \"Binomial_dataset_5_new_exps_combined.RData\")")))
...
Binomial_dataset_5_new_ALL_temp <- Binomial_dataset_5_new_ALL[with(Binomial_dataset_5_new_ALL, order(-Density, CBDA)), ];
names(Binomial_dataset_5_new_ALL_temp)[3]<-c("DensityCBDA")
names(Binomial_dataset_5_new_ALL_temp)[5]<-c("DensityKO")
BEST <- 15
w1 <- c(1:BEST)
w2 <- c(1:BEST)

Binomial_dataset_5_new_ALL_temp$CBDA <- as.numeric(as.character(Binomial_dataset_5_new_ALL_temp$CBDA))

#print(c(Binomial_5_new_ALL_temp$CBDA[w1],Binomial_5_new_ALL_temp$Knockoff[w2]))

a10 <- unique(c(Binomial_dataset_5_new_ALL_temp$CBDA[w1],Binomial_dataset_5_new_ALL_temp$Knockoff[w2]))
