...
#eval(parse(text=paste0("pdf(\"",workspace_directory,"Inference_Generation_",label,".pdf\")")))
...
Ytemp <- Data[,8] # col 9
table(Ytemp)
# This step merges the class LMCI and MCI with AD, making only 2 categories for classification - MN3
Ytemp[which(Ytemp == "LMCI")] <- c("MCI")
Ytemp[which(Ytemp == "MCI")] <- c("AD")
table(Ytemp)
...
print(alpha)
print(a0)
# Randomly select patients for VALIDATION in a balanced way based on the Nrow sample and a0
Ytemp <- as.numeric(Ytemp == "AD")
...
eval(parse(text=paste0("load(\"",workspace_directory,"qa_ALL.RData\")")))
#eval(parse(text=paste0("load(\"",workspace_directory,"/CBDA_SL_M",M,"_miss",misValperc,"_n",range_n,"_k",range_k,"_Light_",label,".RData\")")))
#eval(parse(text=paste0("print(\"",workspace_directory,"CBDA_SL_M",M,"_miss",misValperc,"_n",range_n,"_k",range_k,"_Light_",label,".RData\")")))
...
# a1 <-  min(length(which(Ytemp[n_sub]==1)),a0)
# a2 <-  min(length(which(Ytemp[n_sub]==0)),a0)
# n1 = sample(which(Ytemp[n_sub]==1),a1)
# n2 = sample(which(Ytemp[n_sub]==0),a2)
# n <- c(n1 , n2)
n <- n_sub
...
a=ifelse((is.na(Xtemp)),1,0);
print(c("% of missing data in the Raw Data"))
print(100*sum(a)/(dim(Xtemp)[1]*dim(Xtemp)[2]))
...
dim(Data_unbalanced)
Data_balanced <- SMOTE(Data_unbalanced[,-dim(Data_unbalanced)[2]],Data_unbalanced[,dim(Data_unbalanced)[2]])
dim(Data_balanced$data)
