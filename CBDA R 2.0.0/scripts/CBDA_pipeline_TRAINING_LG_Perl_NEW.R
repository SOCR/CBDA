...
CBDA.pipeline.Perl.LG <- function(job_id , label = "CBDA_package_test" , misValperc = 0 ,
                                  M = 5000 ,  top = 1000, max_covs = 100 , min_covs = 3 ,
  ...
  eval(parse(text=paste0("trainingSet <- c('X",job_id,".txt')")))
  eval(parse(text=paste0("validationSet <- c('X",job_id,"validation.txt')")))
  eval(parse(text=paste0("featureSet <- c('k",job_id,".txt')")))
...
print("Check if there are missing data in X training")
print(100*sum(a)/(dim(data_temp)[1]*dim(data_temp)[2]))

data_temp=Y
a=ifelse((is.na(data_temp)),1,0);
print("Check if there are missing data in Y training")
print(100*sum(a)/(dim(data_temp)[1]*dim(data_temp)[2]))

data_temp=Xpred_norm
a=ifelse((is.na(data_temp)),1,0);
print("Check if there are missing data in X validation")
print(100*sum(a)/(dim(data_temp)[1]*dim(data_temp)[2]))

data_temp=Ypred
a=ifelse((is.na(data_temp)),1,0);
print("Check if there are missing data in Y validation")
print(100*sum(a)/(dim(data_temp)[1]*dim(data_temp)[2]))
...
eval(parse(text=paste0("m_",job_id,"<-stats::coef(SL)")))
...
filename <- file.path(workspace_directory,
                      paste0("CBDA_M",M,"_miss",misValperc,"_",job_id,"_",label,".RData"))
save(list = ls(all.names = TRUE), file=filename)

cat("TRAINING SUCCESSFULL !!!\n\n")
return()

}

## Start the training calling the main training function
CBDA.pipeline.Perl.LG(job_id = job_id, label = label , misValperc = 0 , 
                      M = M , top = top, max_covs = max_covs , min_covs = 3, 
                      algorithm_list = test_example , workspace_directory = workspace_directory)
