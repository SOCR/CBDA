...
output_col=1
time perl -F',' -lane "print join q(,), @F[split "," $output_col,$cols_new]"  $workspaceDirectory/X${jobID}_rows_only_validation_final.txt  > ${workspaceDirectory}X${jobID}validation_final.txt

rm ${workspaceDirectory}X${jobID}_rows_only_final.txt
rm ${workspaceDirectory}X${jobID}_rows_only_validation_final.txt
