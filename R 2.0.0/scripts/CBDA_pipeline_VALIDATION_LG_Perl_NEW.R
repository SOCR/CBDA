...
CBDA.pipeline.Validation.Perl.LG <- function(job_id , label = "CBDA_package_test" , misValperc = 0 ,
                                  M = 9000 ,  top = 1000, max_covs = 100 , min_covs = 5 ,
  ...
  eval(parse(text=paste0("validationSet <- c('X",job_id,"validation_final.txt')")))
...
eval(paste0("newk",job_id,"<-columnNames"))
...
SL <- tryCatch( SuperLearner::SuperLearner(Y , X , newX = Xpred_norm,family=stats::binomial(),
                                           SL.library=algorithm_list,
                                           method="method.NNLS",verbose = FALSE,
                                           control = list(saveFitLibrary = TRUE),
                                           cvControl = list(V=10)), error=function(e) NULL)
if (is.null(SL)){
  eval(parse(text=paste0("m_",job_id,"<-rep(0,length(algorithm_list))")))
  SL_Pred <- NULL
  Classify_MSE<-NULL
  Classify<-NULL
  eval(parse(text=paste0("Accuracy_",job_id,"<- 0.5")))
  eval(parse(text=paste0("MSE_",job_id,"<- 100")))
  rm(SL)
...
CBDA.pipeline.Validation.Perl.LG(job_id = job_id, label = label ,
                      M = M , top = top, max_covs = max_covs , 
