...
hist(Xtemp$Sex, main="SEX Histogram")
hist(Xtemp$Age, main="age Histogram")
hist(Xtemp$TBV, main="TBV Histogram")
hist(Xtemp$GMV, main="GMV Histogram")
hist(Xtemp$WMV, main="WMV Histogram")
hist(Xtemp$CSFV, main="CSFV Histogram")
hist(Xtemp$Value_SA_10_END, main="Value_SA_10_END Histogram")


#set a new matrix for data simulation.
# a is the number of subjects we are going to generate.
a<-50
Xtemp2<-matrix(data=NA,nrow=a,ncol=230)
# generate Age from binomial distribution.
Xtemp2[,1] <- c(ifelse(runif(a)<0.5,0,1))
...
for(i in 2:230){
  Xtemp2[,i]<-rnorm(n=a, mean=mean(Xtemp[,i]), sd=sd(Xtemp[,i]))
}
# According to the original dataset, AGE,TBV,GMV,WMV,CSFV variables should 
# be integer
for(i in 2:6){
  Xtemp2[,i]<-as.integer(Xtemp2[,i])
}
# the values variables contain two digits
for(i in 7:230){
  Xtemp2[,i]<-round(Xtemp2[,i],digits = 2)
}
nam<-names(Xtemp)
colnames(Xtemp2)<-c(nam)

#try to generate by uniform distribution rather than normal distribution
Xtemp3<-matrix(data=NA,nrow=a,ncol=230)
# AGE confroms to binomial distribution as well.
Xtemp3[,1] <- c(ifelse(runif(a)<0.5,0,1))
# Other variables conform to the normal distribution
for(i in 2:230){
  Xtemp3[,i]<-runif(n=a, min=min(Xtemp[,i]), max=max(Xtemp[,i]))
}
# For AGE,TBV,GMV,WMV,CSFV, the results should be integer
for(i in 2:6){
  Xtemp3[,i]<-as.integer(Xtemp3[,i])
}
# For other variables, the results contain two digits
for(i in 7:230){
  Xtemp3[,i]<-round(Xtemp3[,i],digits = 2)
}
nam<-names(Xtemp)
colnames(Xtemp3)<-c(nam)
```

##compare the simulated data and the original data
```{r, echo=TRUE, message=FALSE, warning=FALSE}
#draw QQ-plot for the top 5 variables, which have highest frequency
w=min(which(names(Xtemp)=="CSFV"))
qqplot(Xtemp[,w],Xtemp2[,w],xlab="original data",ylab="simulated data",main="qq plots for CSFV ( the normal distribution), frequency=2449")
qqplot(Xtemp[,w],Xtemp3[,w],xlab="original data",ylab="simulated data",main="qq plots for CSFV ( the uniform distribution),frequency=2449")

w=min(which(names(Xtemp)=="Value_FD_53_END"))
qqplot(Xtemp[,w],Xtemp2[,w],xlab="original data",ylab="simulated data",main="qq plots for Value_FD_53_END ( the normal distribution),frequency=2005")
qqplot(Xtemp[,w],Xtemp3[,w],xlab="original data",ylab="simulated data",main="qq plots for Value_FD_53_END ( the uniform distribution),frequency=2005")

w=min(which(names(Xtemp)=="Value_FD_51_END"))
qqplot(Xtemp[,w],Xtemp2[,w],xlab="original data",ylab="simulated data",main="qq plots for Value_FD_51_END ( the normal distribution),frequency=1339")
qqplot(Xtemp[,w],Xtemp3[,w],xlab="original data",ylab="simulated data",main="qq plots for Value_FD_51_END ( the uniform distribution),frequency=1339")

w=min(which(names(Xtemp)=="Value_FD_8_END"))
qqplot(Xtemp[,w],Xtemp2[,w],xlab="original data",ylab="simulated data",main="qq plots for Value_FD_8_END ( the normal distribution),frequency=1312")
qqplot(Xtemp[,w],Xtemp3[,w],xlab="original data",ylab="simulated data",main="qq plots for Value_FD_8_END ( the uniform distribution),frequency=1312")

w=min(which(names(Xtemp)=="WMV"))
qqplot(Xtemp[,w],Xtemp2[,w],xlab="original data",ylab="simulated data",main="qq plots for WMV  ( the normal distribution),frequency=1171")
qqplot(Xtemp[,w],Xtemp3[,w],xlab="original data",ylab="simulated data",main="qq plots for WMV  ( the uniform distribution),frequency=1171")
```
