...
setwd("~/Documents/NIH-grant/SOCR/GITHUB/PilotTest")
...
source('ipak.R')
ipak(packages)
NeuroIm1_Final_AD_vs_NC_training = read.table("~/Documents/NIH-grant/SOCR/GITHUB/PilotTest/NeuroIm1_rand.txt", header = TRUE)
# Define the temporary output [Ytemp] and input [mp] matrices for the SuperLearner call
Xtemp <- NeuroIm1_Final_AD_vs_NC_training
...
## SUPERLEARNER LOOP
for(j in 1:M) {
  Kcol <- round(K*runif(1,0.3,0.50)) # sample a value from a uniform distribution within 0.15 and 0.3 [number of columns/covariates between 15-30% of the big dataset]
