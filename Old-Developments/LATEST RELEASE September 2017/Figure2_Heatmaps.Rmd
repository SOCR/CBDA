---
title: "CBDA-SL and Knockoff Filter Results on the Binomial Dataset - n=300 p=100, 9000 jobs x 12 experiment - MSE and Accuracy as ranking metrics for the CBDA"
...
# Binomial_dataset_new = read.csv("C:/Users/simeonem/Documents/CBDA-SL/ExperimentsNov2016/Binomial/Binomial_5/Binomial_dataset.txt",header = TRUE)
...
#label=c("Binomial_dataset_new")
label=c("Binomial_dataset_final")
...
eval(parse(text=paste0("save(arguments,workspace_directory,list_exps,label,file= \"~/temp_Binomial_dataset_final.RData\")")))
#print(getwd())
```
...
eval(parse(text=paste0("h=hist(k_top_",top,"_temp_Accuracy, plot = FALSE ,breaks=seq(min(k_top_",top,"_temp_Accuracy)-0.5, max(k_top_",top,"_temp_Accuracy)+0.5, by=1))")))
h$density = h$counts/sum(h$counts)*100
...
# RETRIEVE AND SAVE THE LABELS OF THE TOP [BEST] FEATURES
 Binomial_dataset_final = read.csv("Binomial_dataset_final.txt",header = TRUE)
 #print(dim(Binomial_new))
cols_to_eliminate=c(1)




  # how many top rows to show and use to either calculate FDRs or proceed to the final step
BEST=15;
  Top_features <- NULL
  ...
  Top_features_labels <- names(Binomial_dataset_final[-cols_to_eliminate])[as.numeric(names(Top_features))]
  Top_features_labels_MSE <- names(Binomial_dataset_final[-cols_to_eliminate])[as.numeric(names(Top_features_MSE))]
  
  eval(parse(text=paste0("Top_",BEST,"_features_labels <- names(Binomial_dataset_final[-cols_to_eliminate])[as.numeric(names(Top_features)[1:",BEST,"])]")))
  eval(parse(text=paste0("Top_",BEST,"_features_labels_MSE <- names(Binomial_dataset_final[-cols_to_eliminate])[as.numeric(names(Top_features_MSE)[1:",BEST,"])]")))
...
#print(paste("EXPERIMENT",i),quote = FALSE)
#print.table(arguments[i,])
print(c("M","Top-ranked"))
print(c(s,top))
print("Accuracy Count")
print(length(which(qa_ALL$Accuracy[1:BEST] %in% nonzero)))
print("MSE Count")
print(length(which(qa_ALL$MSE[1:BEST] %in% nonzero)))
print("Knockoff Count")
print(length(which(qa_ALL$Knockoff[1:BEST] %in% nonzero)))
print("Nonzero Features")
print(nonzero)
#eval(parse(text=paste0("Binomial_dataset_final_exp_",i,"<-qa")))
eval(parse(text=paste0("Binomial_dataset_final_exp_",i,"<-qa_ALL")))
...
#eval(parse(text=paste0("save(Binomial_dataset_final_exp_",i,",arguments,workspace_directory,list_exps,label,file= \"Binomial_dataset_final_exp_",i,".RData\")")))

eval(parse(text=paste0("save(Binomial_dataset_final_exp_",i,",arguments,workspace_directory,list_exps,label,qa_ALL,k_top_",top,"_temp_Accuracy,k_top_",top,"_temp,k_top_",top,"_temp_exp_",i,",k_top_",top,"_temp_Accuracy_exp_",i,",KO_sub_exp_",i,", file= \"Binomial_dataset_final_exp_",i,".RData\")")))


rm(list = ls())
eval(parse(text=paste0("load(\"~/temp_Binomial_dataset_final.RData\")")))
cat("\n\n\n\n\n\n")
}
```
...
for (i in list_exps)
  {
  eval(parse(text=paste0("load('Binomial_dataset_final_exp_",i,".RData')")))
...
# eval(parse(text=paste0("load(\"~/temp_Binomial_dataset_new.RData\")")))
# Binomial_dataset_new_ALL <- NULL
# for (i in list_exps) {
#   eval(parse(text=paste0("load(\"Binomial_dataset_new_exp_",i,".RData\")")))
#   eval(parse(text=paste0("Binomial_dataset_new_ALL <- rbind(Binomial_dataset_new_ALL,Binomial_dataset_new_exp_",i,")")))
# }
# eval(parse(text=paste0("save(Binomial_dataset_new_ALL,arguments,workspace_directory,list_exps,label,file= \"Binomial_dataset_new_exps_combined.RData\")")))
# 
...
# Binomial_dataset_new_ALL_temp <- Binomial_dataset_new_ALL[with(Binomial_dataset_new_ALL, order(-Density, CBDA)), ];
# names(Binomial_dataset_new_ALL_temp)[3]<-c("DensityCBDA")
# names(Binomial_dataset_new_ALL_temp)[5]<-c("DensityKO")
# BEST <- 15
# w1 <- c(1:BEST)
# w2 <- c(1:BEST)
# 
# Binomial_dataset_new_ALL_temp$CBDA <- as.numeric(as.character(Binomial_dataset_new_ALL_temp$CBDA))
# 
# #print(c(Binomial_5_new_ALL_temp$CBDA[w1],Binomial_5_new_ALL_temp$Knockoff[w2]))
# 
# a10 <- unique(c(Binomial_dataset_new_ALL_temp$CBDA[w1],Binomial_dataset_new_ALL_temp$Knockoff[w2]))
# print("Top Features Selected across multiple experiments,shared between CBDA-SL and Knockoff filter")
# print(a10)


# dataset_file=c("Binomial_dataset.txt")
...
# The features listed above are then used to run a final analysis applying both the CBDA-SL and the knockoff filter. 
# The ONLY features used for analysis are the ones listed above. 
# A final summary of the accuracy of the overall procedure is determined by using the CDBA-SL object on the subset of 
# subjects held off for prediction. The predictions are then used to generate the confusion matrix.
# We basically combine the CBDA-SL & Knockoff Filter algorithms to first select the top features during the first round.
# Then, the second stage uses the top features to run a final predictive modeling step that can ultimately be tested 
