---
title: "CBDA-SL and Knockoff Filter Results on the Binomial Dataset - n=300 p=100, 9000 jobs x 6 experiment - MSE and Accuracy as ranking metrics for the CBDA"
...
eval(parse(text=paste0("save(arguments,workspace_directory,list_exps,label,file= \"~/temp_Binomial_dataset_new.RData\")")))
#print(getwd())
```
...
# RETRIEVE AND SAVE THE LABELS OF THE TOP [BEST] FEATURES
 Binomial_dataset_new = read.csv("Binomial_dataset_final.txt",header = TRUE)
...
cols_to_eliminate=c(1)
  ...
  Top_features_labels <- names(Binomial_dataset_new[-cols_to_eliminate])[as.numeric(names(Top_features))]
  Top_features_labels_MSE <- names(Binomial_dataset_new[-cols_to_eliminate])[as.numeric(names(Top_features_MSE))]
  
  eval(parse(text=paste0("Top_",BEST,"_features_labels <- names(Binomial_dataset_new[-cols_to_eliminate])[as.numeric(names(Top_features)[1:",BEST,"])]")))
  eval(parse(text=paste0("Top_",BEST,"_features_labels_MSE <- names(Binomial_dataset_new[-cols_to_eliminate])[as.numeric(names(Top_features_MSE)[1:",BEST,"])]")))
...
eval(parse(text=paste0("Binomial_dataset_new_exp_",i,"<-qa")))
...
eval(parse(text=paste0("save(Binomial_dataset_new_exp_",i,",arguments,workspace_directory,list_exps,label,file= \"Binomial_dataset_new_exp_",i,".RData\")")))
# print(q)
rm(list = ls())
