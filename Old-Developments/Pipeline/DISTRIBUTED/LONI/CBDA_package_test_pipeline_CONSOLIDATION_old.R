...
CBDA_Consolidation.pipeline <- function(top , max_covs , M , misValperc ,
  ...
  for (i in 1:M){
    eval(parse(text=paste0("m_training[",i,",] <- m_",i)))
  }
  ...
  save(qa_ALL,label,workspace_directory,M,range_k,range_n,misValperc,m_training,
  ...
  filename_1 <- file.path(workspace_directory,
                        paste0(label,"_m_training.RData"))
  save(m_training, file = filename_1)
