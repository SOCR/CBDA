---
title: "CBDA-SL and Knockoff Filter Results on the Null Dataset - n=300 p=1500, 9000 jobs x 8 experiments - MSE and Accuracy as ranking metrics for CBDA"
...
# Null_dataset_5_new = read.csv("C:/Users/simeonem/Documents/CBDA-SL/ExperimentsNov2016/Null/Null_5/Null_dataset_5_5.txt",header = TRUE)
...
#list_exps = c(1:3,5:7,9,11) #DONE
list_exps = c(1:2,5,7,11) # useful, since the KO doesn't work for n < p
```

```{r Save basic info in a temp file to speed up the script while looping through the experiments, echo=FALSE}
#label=c("Null_dataset_5_new")
label=c("Null_dataset_5_final")
#label=c("Null_dataset_5_1_final")
...
eval(parse(text=paste0("save(arguments,workspace_directory,list_exps,label,file= \"~/temp_Null_dataset_5_new.RData\")")))
#print(getwd())
```
...
# RETRIEVE AND SAVE THE LABELS OF THE TOP [BEST] FEATURES
 Null_dataset_5_new = read.csv("Null_dataset_5_final.txt",header = TRUE)
  ...
  Top_features_labels <- names(Null_dataset_5_new[-cols_to_eliminate])[as.numeric(names(Top_features))]
  Top_features_labels_MSE <- names(Null_dataset_5_new[-cols_to_eliminate])[as.numeric(names(Top_features_MSE))]
  
  eval(parse(text=paste0("Top_",BEST,"_features_labels <- names(Null_dataset_5_new[-cols_to_eliminate])[as.numeric(names(Top_features)[1:",BEST,"])]")))
  eval(parse(text=paste0("Top_",BEST,"_features_labels_MSE <- names(Null_dataset_5_new[-cols_to_eliminate])[as.numeric(names(Top_features_MSE)[1:",BEST,"])]")))
...
#Null_dataset_5_final_MSE
#print("Nonzero Features")
#print(nonzero)
eval(parse(text=paste0("Null_dataset_5_final_exp_",i,"<-qa_ALL")))
...
eval(parse(text=paste0("save(Null_dataset_5_final_exp_",i,",arguments,workspace_directory,list_exps,label,qa_ALL,k_top_",top,"_temp_Accuracy,k_top_",top,"_temp,k_top_",top,"_temp_exp_",i,",k_top_",top,"_temp_Accuracy_exp_",i,",KO_sub_exp_",i,", file= \"Null_dataset_5_final_exp_",i,".RData\")")))
# print(q)
rm(list = ls())
eval(parse(text=paste0("load(\"~/temp_Null_dataset_5_new.RData\")")))
cat("\n\n\n\n\n\n")
}
```
...
for (i in list_exps)
  {
  eval(parse(text=paste0("load('Null_dataset_5_final_exp_",i,".RData')")))
