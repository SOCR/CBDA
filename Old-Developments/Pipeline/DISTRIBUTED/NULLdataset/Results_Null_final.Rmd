---
title: "CBDA-SL and Knockoff Filter Results on the Null Dataset - n=300 p=100, 9000 jobs x 10 experiments - MSE and Accuracy as ranking metrics for CBDA"
...
eval(parse(text=paste0("save(arguments,workspace_directory,list_exps,label,file= \"~/temp_Null_dataset_new.RData\")")))
#print(getwd())
```
...
# RETRIEVE AND SAVE THE LABELS OF THE TOP [BEST] FEATURES
 Null_dataset_new = read.csv("Null_dataset_final.txt",header = TRUE)
  ...
  Top_features_labels <- names(Null_dataset_new[-cols_to_eliminate])[as.numeric(names(Top_features))]
  Top_features_labels_MSE <- names(Null_dataset_new[-cols_to_eliminate])[as.numeric(names(Top_features_MSE))]
  
  eval(parse(text=paste0("Top_",BEST,"_features_labels <- names(Null_dataset_new[-cols_to_eliminate])[as.numeric(names(Top_features)[1:",BEST,"])]")))
  eval(parse(text=paste0("Top_",BEST,"_features_labels_MSE <- names(Null_dataset_new[-cols_to_eliminate])[as.numeric(names(Top_features_MSE)[1:",BEST,"])]")))
...
#Null_dataset_final_MSE
#print("Nonzero Features")
#print(nonzero)
eval(parse(text=paste0("Null_dataset_final_exp_",i,"<-qa_ALL")))
...
eval(parse(text=paste0("save(Null_dataset_final_exp_",i,",arguments,workspace_directory,list_exps,label,qa_ALL,k_top_",top,"_temp_Accuracy,k_top_",top,"_temp,k_top_",top,"_temp_exp_",i,",k_top_",top,"_temp_Accuracy_exp_",i,",KO_sub_exp_",i,", file= \"Null_dataset_final_exp_",i,".RData\")")))
# print(q)
rm(list = ls())
eval(parse(text=paste0("load(\"~/temp_Null_dataset_new.RData\")")))
cat("\n\n\n\n\n\n")
}
```
...
for (i in list_exps)
  {
  eval(parse(text=paste0("load('Null_dataset_final_exp_",i,".RData')")))
