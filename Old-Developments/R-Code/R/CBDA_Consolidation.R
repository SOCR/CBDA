#' @title
#' CBDA Consolidation function for Compressive Big Data Analytics
#'
#' @description
...
CBDA_Consolidation <- function(top , max_covs , M , misValperc ,
  ...
  for (j in 1:M){
    ...
    load(filename)
    #eval(parse(text=paste0("load(\"",workspace_directory,"/CBDA_SL_M",M,"_miss",misValperc,
    #                       "_n",range_n,"_k",range_k,"_Light_",j,"_",label,".RData\")")))
    }
  ...
  if (N_cores>1)
  {
    ...
    for (i in 1:M){
      eval(parse(text=paste0("m[",i,",] <- m_",i)))
    }
  }
